...
export class MCPToolkit extends BaseToolkit {
    ...
    async get_tools(): Promise<Tool[]> {
        ...
        const errors = res.filter((r: PromiseSettledResult<any>) => r.status === 'rejected')
        if (errors.length !== 0) {
            console.error('MCP Tools falied to be resolved', errors)
        }
        const successes = res.filter((r: PromiseSettledResult<any>) => r.status === 'fulfilled').map((r: PromiseFulfilledResult<any>) => r.value)
        return successes
    }
}
