...
let inboundConfig: InboundConfig | null = null
...
async function getOrCreateSharedServer(): Promise<{ server: http.Server; port: number; wss: any }> {
    ...
    const server = http.createServer((req, res) => {
        ...
        // Match /inbound — Twilio incoming call webhook
        if (url.match(/^\/inbound/)) {
            if (!inboundConfig) {
                // Return a valid TwiML response so Twilio doesn't mark the call as Busy
                const notReadyTwiml = [
                    '<?xml version="1.0" encoding="UTF-8"?>',
                    '<Response>',
                    '  <Say>The assistant is not available yet. Please run an outbound call from Flowise first, then try again.</Say>',
                    '</Response>'
                ].join('\n')
                res.writeHead(200, { 'Content-Type': 'text/xml' })
                res.end(notReadyTwiml)
                console.warn('[TwilioVoiceCall] Inbound call received but no config registered yet. Run an outbound call first.')
                return
            }
            let body = ''
            ...
            req.on('end', () => {
                try {
                    ...
                    const callFrom = params.get('From') || 'unknown'
                    ...
                    manager.createSession(sessionId, {
                        ...
                        welcomeGreeting: cfg.welcomeGreeting,
                        maxDurationSeconds: cfg.maxDuration,
                        llmModel: cfg.llmModel
                    })

                    const wsUrl = `${cfg.publicBaseUrl.replace(/^https?:\/\//, 'wss://')}/ws/${sessionId}`
                    let crAttrs = `url="${wsUrl}" welcomeGreeting="${cfg.welcomeGreeting}"`
                    crAttrs += ` ttsProvider="${cfg.ttsProvider}"`
                    crAttrs += ` transcriptionProvider="${cfg.sttProvider}"`
                    crAttrs += ` language="${cfg.language}"`
                    if (cfg.ttsVoice) crAttrs += ` voice="${cfg.ttsVoice}"`
                    ...
                    console.log(`[TwilioVoiceCall] Inbound call from ${callFrom} (SID: ${callSid}) → session ${sessionId}`)
                    console.log(`[TwilioVoiceCall] Inbound WebSocket: ${wsUrl}`)

                    // Cleanup after maxDuration + buffer
                    setTimeout(() => {
                        pendingSessions.delete(sessionId)
                        manager.removeSession(sessionId)
                        console.log(`[TwilioVoiceCall] Inbound session ${sessionId} cleaned up`)
                    }, (cfg.maxDuration + 120) * 1000)

                    res.writeHead(200, { 'Content-Type': 'text/xml' })
                    res.end(twiml)
                } catch (e) {
                    console.error('[TwilioVoiceCall] Inbound handler error:', e)
                    res.writeHead(500)
...
class TwilioVoiceCall_AgentFlows implements INode {
    ...
    async run(nodeData: INodeData, _input: string | Record<string, any>, options: ICommonObject): Promise<any> {
        ...
        // ------------------------------------------------------------------
        // 3b. Register inbound config (so incoming calls use the same settings)
        // ------------------------------------------------------------------
        inboundConfig = {
            systemPrompt,
            welcomeGreeting,
        ...
        console.log(`[TwilioVoiceCall] ── Inbound webhook    : ${baseUrl}/inbound  ← set this in Twilio Console`)
...
// Eagerly start the bridge server when this module loads so inbound calls
// work as soon as Flowise starts — no outbound call required to open the port.
;(async () => {
    try {
        const { port } = await getOrCreateSharedServer()
        console.log(`[TwilioVoiceCall] Bridge server pre-started on port ${port} — ready for inbound calls`)
    } catch (e) {
        console.error('[TwilioVoiceCall] Could not pre-start bridge server:', e)
    }
})()
